...
//		어떤 요청이 들어왔나~~ 받아보자
		...
		System.out.println(context);		// => /insert.nhn 뭐가됐든 주소창에 요청된 모든 *.nhn 파일이 열리는걸 확인할 수 있지!
		...
		switch (context) {
			...
			case "/insertOK.nhn":
//				insert.jsp에서 테이블에 저장할 데이터를 입력하고 submit 버튼을 클릭하면 폼에 입력된 정보가 
//				컨트롤러의 doPost() 메소드의 HttpServletRequest 인터페이스 타입의 객체인 request로 전달된다.
//				doPost() 메소드에서는 request 객체에 저장된 데이터를 가지고 actionDo() 메소드를 실행하므로
//				insert.jsp에서 폼에 입력한 데이터는 actionDo() 메소드의 request 객체로 저장된다.
//				insert.jsp에서 폼에 입력한 데이터를 테이블에 저장하는 메소드를 호출한다.
//				service -> DAO -> .xml 을 거쳐서 진행되는 작업 알지?? 
				service.insert(request, response);
				viewPage += "index";
				break;	
				
			case "/list.nhn":
//				원래는 list.jsp랑 listView.jsp를 따로 만들어서 했지만 이젠 아니지롱!(서비스클래스에서 할거야!)
//				브라우저에 출력할 1페이지 분량의 글과 페이지 작업에 사용할 8개의 변수가 
...
//				답글이 브라우저에 표시되는 위치를 정하기 위해 조건에 만족하는 seq값을 1씩 증가시킨 후 답글을 저장한다.
