...
//	얘도 몇 번 불러서 사용하니까, 멤버변수로 만들어두고 사용하자~~
	private MvcboardDAO dao = MvcboardDAO.getInstance();
	
//	컨트롤러에 insertOK.nhn으로 요청이 들어오면 컨트롤러에서 호출하는 메소드로 
//	테이블에 저장할 메인글이 저장된 request 객체를 넘겨받고 mapper를 얻어온 후
...
//		request 객체로 넘어온 insert.jsp에서 폼에 이벽한 데이터를 받아서 MvcboardVO 클래스 객체에 저장한다.
		String name = request.getParameter("name");
		String subject = request.getParameter("subject");
		String content = request.getParameter("content");
		MvcboardVO vo = new MvcboardVO(name, subject, content);
		
//		DAO클래스의 insert.jsp에서 입력한 데이터를 테이블에 저장하는 insert sql 명령을 실행하는 메소드를 호출한다.
...
//	컨트롤러에 list.nhn으로 요청이 들어오면 컨트롤러에서 호출하는 메소드로 mapper를 얻어온 후
//	DAO클래스의 브라우저에 출력할 1페이지 분량의 글목록과 페이지 작업에 사용할 8개의 변수가 저장된 클래스 객체를 만들어
...
//		원래는 list.jsp랑 listView.jsp를 따로 만들어서 했지만 이젠 여기서 할거야!
...
//	컨트롤러에 increment.nhn으로 요청이 들어오면 컨트롤러에서 호출하는 메소드로 조회수를 넘겨받고 mapper를 얻어온 후
//	DAO클래스의 조회수를 증가시키는 메소드	
...
//	컨트롤러에 contentView.nhn으로 요청이 들어오면 컨트롤러에서 호출하는 메소드로 조회수를 증가시킨 글번호를 넘겨받고 mapper를 얻어온 후
//	DAO클래스의 조회수를 증가시키는 메소드		
...
//		브라우저에 표시할 글이 저장된 객체, 작업 후 돌아갈 페이지 번호, 
//		줄바꿈에 사용할 이스케이프 시퀀스를 request 영역에 저장한다.
...
//	컨트롤러에 delete.nhn으로 요청이 들어오면 컨트롤러에서 호출하는 메소드로 삭제할 글번호를 넘겨받고 mapper를 얻어온 후
//	DAO클래스의 글 1건을 삭제하는 delete sql 명령을 실행하는 메소드			
...
//	컨트롤러에 update.nhn으로 요청이 들어오면 컨트롤러에서 호출하는 메소드로 수정할 글번호와 정보를 넘겨받고 mapper를 얻어온 후
//	DAO클래스의 글 1건을 수정하는 update sql 명령을 실행하는 메소드				
	public void update(HttpServletRequest request, HttpServletResponse response) {
		System.out.println("MvcboardService 클래스의 update() 메소드");
		SqlSession mapper = MySession.getSession();
		
//		request 객체로 넘어오는 수정할 글번호와 정보를 받아서 MvcboardVO 클래스 객체에 저장한 후 글을 수정하는 메소드를 실행한다.
		int idx = Integer.parseInt(request.getParameter("idx"));
		String subject = request.getParameter("subject");
		String content = request.getParameter("content");
...
//	replyInsert.nhn으로 요청이 들어오면 컨트롤러에서 호출하는 메소드로 답글 정보를 넘겨받고 mapper를 얻어온 후
//	조건에 만족하는 seq를 1씩 증가시키는 메소드를 호출한 후 답글을 저장하는 insert sql 명령을 실행하는 메소드를 호출하는 메소드	
...
//		넘겨받은 답글 정보를 MvcboardVO 클래스 객체에 저장한다. => request 객체로 넘겨받은 lev와 seq는 1씩 증가 시킨다.
		MvcboardVO vo = new MvcboardVO(name, subject, content);
		vo.setIdx(idx);
		vo.setRef(ref);
		vo.setLev(lev + 1);
